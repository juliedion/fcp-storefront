...
export default function FindCard({ item }: { item: Find }) {
  ...
  const rawCardDescription =
    ...
    (item.productSource === "zendrop"
      ? `${item.title} is a fun, practical Fort Crazypants find selected for everyday family life.`
      : "A fun find worth checking out.");

  const cardDescription = rawCardDescription
